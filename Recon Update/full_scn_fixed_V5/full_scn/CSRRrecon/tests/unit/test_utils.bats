...
# Unit tests for CSRRrecon utility functions
...
setup() {
    ...
    source "$project_root/CSRRrecon.cfg" 2>/dev/null || true
    export SCRIPTPATH="$project_root"
    source "$project_root/CSRRrecon.sh" --source-only
}
...
@test "check_secrets_permissions warns on world-readable file" {
    ...
    run bash -c "
        source '$SCRIPTPATH/CSRRrecon.sh' --source-only
