    ...
    if { [[ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ]] || [[ $DIFF == true ]]; } && [[ $WEBSCREENSHOT == true ]]; then
        if [[ $AXIOM != true ]]; then
            if ! command -v gowitness >/dev/null 2>&1; then
                _print_msg WARN "${FUNCNAME[0]}: gowitness binary not found in PATH - install gowitness first"
                return 0
            fi
        fi
        ...
        # Run gowitness or axiom-scan based on AXIOM flag
        if [[ $AXIOM != true ]]; then
            if [[ -s "webs/webs_all.txt" ]]; then
                run_command gowitness scan file -f webs/webs_all.txt --screenshot-path screenshots/ --no-db 2>>"$LOGFILE" >/dev/null
            fi
        else
            if [[ -s "webs/webs_all.txt" ]]; then
                run_command axiom-scan webs/webs_all.txt -m gowitness -o screenshots "$AXIOM_EXTRA_ARGS" 2>>"$LOGFILE" >/dev/null
            fi
        fi
...
function nuclei_check() {
    # Vulnerability scanning removed — nuclei_check is disabled.
    skip_notification "disabled"
}
