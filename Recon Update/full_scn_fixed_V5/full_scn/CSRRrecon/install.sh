...
CONFIG_FILE="./CSRRrecon.cfg"

if [[ ! -f $CONFIG_FILE ]]; then
    printf "%b[!] Config file CSRRrecon.cfg not found.%b\n" "$bred" "$reset"
    exit 1
fi

# shellcheck source=./CSRRrecon.cfg
...
# Ensure Go environment is available from the start (non-interactive shells often miss this)
export GOROOT="${GOROOT:-/usr/local/go}"
export GOPATH="${GOPATH:-$HOME/go}"
export PATH="$GOPATH/bin:$GOROOT/bin:$HOME/.local/bin:$PATH"
# Prevent git from hanging waiting for credentials in non-interactive mode
...
declare -A repos=(
    ...
    # ["CSRRrecon_ai"]="six2dez/CSRRrecon_ai"  # REMOVED: repo deleted from GitHub (HTTP 404)
...
function install_tools() {
    ...
    for gotool in "${!gotools[@]}"; do
        ...
        # Use direct env instead of bash -lc to avoid interactive credential prompts
        if GIT_TERMINAL_PROMPT=0 GOPATH="${GOPATH:-$HOME/go}" GOROOT="${GOROOT:-/usr/local/go}" PATH="${GOPATH:-$HOME/go}/bin:${GOROOT:-/usr/local/go}/bin:$HOME/.local/bin:$PATH" q bash -c "$_go_cmd"; then
            ((++go_ok))
    ...
    for repo in "${!repos[@]}"; do
        ...
        if [[ ! -d "${dir}/${repo}/.git" ]]; then
            ...
            GIT_TERMINAL_PROMPT=0 retry 3 3 q_to 180 git clone --filter="blob:none" "https://github.com/${repos[$repo]}" "${dir}/${repo}"
            exit_status=$?
...
function check_updates() {
    printf "%bRunning: Looking for new CSRRrecon version%b\n" "$bblue" "$reset"
    ...
    if { [[ -n $TIMEOUT_CMD ]] && $TIMEOUT_CMD 10 git fetch; } || git fetch; then
        ...
        if [[ $HEADHASH != "$UPSTREAMHASH" ]]; then
            ...
            if git status --porcelain | grep -q 'CSRRrecon.cfg$'; then
                cfg_backup_ts=$(date +%Y%m%d_%H%M%S)
                cfg_backup="CSRRrecon.cfg.bak.${cfg_backup_ts}"
                cp CSRRrecon.cfg "$cfg_backup"
                printf "%bCSRRrecon.cfg has been backed up to %s%b\n" "$yellow" "$cfg_backup" "$reset"
            fi
            ...
            if [[ -n $cfg_backup && -f CSRRrecon.cfg ]]; then
                mkdir -p .tmp
                cfg_diff_file=".tmp/CSRRrecon_cfg_diff_${cfg_backup_ts}.patch"
                if diff -u "$cfg_backup" CSRRrecon.cfg >"$cfg_diff_file"; then
            ...
            printf "%bCSRRrecon is already up to date!%b\n" "$bgreen" "$reset"
            # If config is locally modified, still provide a diff against current default
            if ! git diff --quiet -- CSRRrecon.cfg; then
                local cfg_diff_ts cfg_diff_file
                cfg_diff_ts=$(date +%Y%m%d_%H%M%S)
                mkdir -p .tmp
                cfg_diff_file=".tmp/CSRRrecon_cfg_diff_${cfg_diff_ts}.patch"
                if git diff HEAD --unified -- CSRRrecon.cfg >"$cfg_diff_file"; then
                    printf "%bLocal CSRRrecon.cfg differs from default; diff saved to %s.%b\n" "$yellow" "$cfg_diff_file" "$reset"
                fi
            fi
        fi
    else
    ...
    if [[ $install_golang == "true" ]]; then
        ...
        if [[ -n $current_version && $version == "$current_version" ]]; then
            ...
            if [[ -d /usr/local/go ]]; then
                go_backup="/usr/local/go.CSRRrecon.$(date +%s)"
        ...
        if [[ -n ${profile_shell:-} ]]; then
            ...
            local marker="# Golang environment variables (CSRRrecon)"

            # Remove ALL previous golang env blocks (old-style and CSRRrecon-style)
        ...
        msg_warn "Golang will not be configured according to the user's preferences (install_golang=false in CSRRrecon.cfg)."
    fi

    if ! command -v go &>/dev/null; then
        msg_err "[!] Go binary not found in PATH. Please install Go or enable install_golang in CSRRrecon.cfg."
        return 1
    fi
...
# Setup CSRRrecon venv for python-only helpers (e.g., getjswords)
function setup_CSRRrecon_venv() {
    local root_dir="${SCRIPTPATH:-$(pwd)}"
    local venv_dir="${root_dir}/.venv"
    if ! command -v python3 >/dev/null 2>&1; then
        msg_warn "[!] python3 not found; skipping CSRRrecon venv setup"
        return 0
    fi
    if [[ ! -d "$venv_dir" ]]; then
...
- SSRF Server (COLLAB_SERVER in CSRRrecon.cfg or env var)
- Blind XSS Server (XSS_SERVER in CSRRrecon.cfg or env var)
- notify (${HOME}/.config/notify/provider-config.yaml)
- WHOISXML API (WHOISXML_API in CSRRrecon.cfg or env var)
EOF
    )"
...
function handle_install_arguments() {
    printf "\n%bCSRRrecon installer/updater script%b\n" "$bgreen" "$reset"
    ...
    if [[ "$(id -u)" -eq 0 ]]; then
        ...
        if ! sudo -n true 2>/dev/null; then
            ...
            printf "%becho \"%s  ALL=(ALL:ALL) NOPASSWD: ALL\" | sudo tee /etc/sudoers.d/CSRRrecon%b\n\n" "$bred" "$USER" "$reset"
        fi
        SUDO="sudo"
    fi
}
