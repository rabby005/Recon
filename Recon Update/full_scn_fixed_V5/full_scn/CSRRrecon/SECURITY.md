...
- Subject: `[CSRRrecon Security] <brief description>`
...
CSRRrecon handles API keys and credentials in three ways (in order of precedence):
...
Create a `secrets.cfg` file in the CSRRrecon root directory. It is automatically sourced at startup and is listed in `.gitignore`:
...
| `CSRRrecon.cfg` (with keys) | Configuration with API keys filled in |
...
CSRRrecon sanitizes all user-supplied input to prevent command injection:
...
These variables in `CSRRrecon.cfg` control security and operational safety:
