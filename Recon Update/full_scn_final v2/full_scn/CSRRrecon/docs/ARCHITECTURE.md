# CSRRrecon Architecture

## Overview

CSRRrecon is a modular reconnaissance framework. The codebase is organized as follows:

```
CSRRrecon/
├── CSRRrecon.sh          # Main entry point, argument parsing, orchestration
├── CSRRrecon.cfg         # Main configuration file (all settings)
...
Modules are sourced in this specific order (defined in `CSRRrecon.sh`):
...
- `$SCRIPTPATH` - Path to CSRRrecon installation
- `$domain` - Target domain
- `$dir` - Output directory for current scan

### Configuration (from CSRRrecon.cfg)
