...
# SC2154: Variables like bred, reset, bgreen, yellow are defined in CSRRrecon.cfg
...
# Welcome to CSRRrecon main script
#	 ██▀███  ▓█████  ▄████▄   ▒█████   ███▄    █   █████▒▄▄▄█████▓ █     █░
#	▓██ ▒ ██▒▓█   ▀ ▒██▀ ▀█  ▒██▒  ██▒ ██ ▀█   █ ▓██   ▒ ▓  ██▒ ▓▒▓█░ █ ░█░
#	▓██ ░▄█ ▒▒███   ▒▓█    ▄ ▒██░  ██▒▓██  ▀█ ██▒▒████ ░ ▒ ▓██░ ▒░▒█░ █ ░█
#	▒██▀▀█▄  ▒▓█  ▄ ▒▓▓▄ ▄██▒▒██   ██░▓██▒  ▐▌██▒░▓█▒  ░ ░ ▓██▓ ░ ░█░ █ ░█
...
. "${SCRIPTPATH}"/CSRRrecon.cfg || {
    _print_error "Error importing CSRRrecon.cfg"
    exit 1
}
...
if [[ -s $CUSTOM_CONFIG ]]; then
    # shellcheck source=/home/six2dez/Tools/CSRRrecon/custom_config.cfg
    . "${CUSTOM_CONFIG}" || {
...
case $opt_mode in
    ...
    'w')
        if [[ -n $list ]]; then
            ...
            _print_error "Web mode needs a website list file as target (./CSRRrecon.sh -l target.txt -w)"
            exit $E_INVALID_INPUT
        fi
        webs_menu
        exit 0
