#!/usr/bin/env bash
# lib/parallel.sh - Parallel execution utilities for CSRRrecon
# Part of CSRRrecon refactoring - Phase 3
...
parallel_funcs() {
    ...
    if [[ -n "${dir:-}" ]]; then
        ...
        parallel_log_root="$(mktemp -d /tmp/CSRRrecon_parallel.XXXXXX)"
    fi
