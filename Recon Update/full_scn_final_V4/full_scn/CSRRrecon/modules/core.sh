#!/bin/bash
# CSRRrecon - Core framework module
...
function tools_installed() {
    ...
    declare -A tools_files=(
        ...
        ["CSRRrecon_ai"]="${tools}/CSRRrecon_ai/CSRRrecon_ai.py"
        ["CSRRrecon_ai_python"]="${tools}/CSRRrecon_ai/venv/bin/python3"
...
function sendToNotify {
    if [[ -z $1 ]]; then
        ...
        if [[ -n $slack_channel ]] && [[ -n $slack_auth ]]; then
            ...
            run_command curl -F file=@${1} -F "initial_comment=CSRRrecon zip file" -F channels=${slack_channel} -H "Authorization: Bearer ${slack_auth}" https://slack.com/api/files.upload 2>>"$LOGFILE" >/dev/null
        fi
    fi
}
...
function plugins_emit() {
    local event="$1"
    shift || true
    if declare -F CSRRrecon_plugins >/dev/null 2>&1; then
        CSRRrecon_plugins "$event" "$@" || true
    fi
}
...
<head>
  ...
  <title>CSRRrecon Report - ${domain}</title>
  <style>
...
<body>
  <div class="wrap">
    <h1>CSRRrecon Consolidated Report</h1>
