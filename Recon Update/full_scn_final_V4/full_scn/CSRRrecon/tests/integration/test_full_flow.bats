...
# Integration tests for CSRRrecon full workflow
...
setup() {
    ...
    export TEST_DIR="$BATS_TEST_TMPDIR/CSRRrecon_test"
    mkdir -p "$TEST_DIR"
...
# Helper to source CSRRrecon without running main
source_CSRRrecon() {
    # shellcheck source=/dev/null
    source "$SCRIPTPATH/CSRRrecon.sh" --source-only 2>/dev/null || true
}
...
@test "full flow: dry-run mode prevents execution" {
    run timeout 5 bash "$SCRIPTPATH/CSRRrecon.sh" -d test.com --dry-run 2>&1
